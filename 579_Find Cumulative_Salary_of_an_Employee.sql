...
Write a solution to calculate the cumulative salary summary for every employee in a single unified table.Compensation & Benefits
...
The result format is in the following example.Management
...
WITH ExcludeMaxMonth AS (
  ...
  FROM CummulativeSum 
  WHERE (id, 'month') 
  NOT IN (SELECT id, max_month FROM ExcludeMaxMonth) 
